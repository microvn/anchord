...
- `refund_window_expired` — the capture is older than the 120-day processor window.
- `idempotency_conflict` — the key was reused with a different body.

## Open questions

One decision is now settled: a `failed` refund never auto-retries — the caller always issues a fresh call once the cause is resolved. Still open for v1 is how partial refunds will pin to a capture; the idempotency model above should extend without a breaking change. It is tracked for the v1 review.
