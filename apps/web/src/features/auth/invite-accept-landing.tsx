...
export function InviteAcceptLanding() {
  ...
  if (wrongAccount) {
    return (
      <main className="mx-auto max-w-md px-4 py-16 text-center">
        <p data-testid="doc-invite-wrong-account" className="font-serif text-lg text-ink">
  ...
  return (
    <main className="mx-auto max-w-md px-4 py-16 text-center">
      <h1 className="font-serif text-xl text-ink">You've been invited to a document</h1>
      <p className="mt-1 text-sm text-muted">Accept to get access to the document.</p>

      {error && (
        <p role="alert" data-testid="doc-invite-error" className="mt-3 text-sm text-error">
          {error}
        </p>
      )}
