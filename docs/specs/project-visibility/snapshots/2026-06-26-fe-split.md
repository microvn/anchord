...
**Date:** 2026-06-26
**Ref:** fe-split
**Reason:** M6 — C-012 gains the default-project demote guard (S-007 build S3 signal); plus a new `## Linked Fields` block as this spec becomes the producer side of a scope-by-layer split with `project-visibility-fe`.
...
- C-012: A live project's `owner_id` is never null — when a member is removed, their owned projects
  ...
  to everyone under C-002). (AS-027, AS-028)
