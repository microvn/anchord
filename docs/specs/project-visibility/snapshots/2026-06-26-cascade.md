...
default project is private (its shell is mine to see); a project I deliberately create is public by
default, but I may choose private at create time — on the web (the New Project dialog, see
`project-visibility-fe:S-005`) or over MCP. An agent over MCP creates public by default but may
specify private.
...
AS-015: The projects list carries the visibility + default + toggle-affordance data
- **Given:** a member's Projects page with their default (private) project, a public project they own, and a public project they neither own nor admin
- **When:** the list loads
- **Then:** each project row carries `isDefault` + `visibility` so the UI shows the Default badge + a private/public indicator, AND a server-computed `canToggleVisibility` flag (true where the viewer may change this project's visibility — owner always, admin only on a public project — reflecting the same gate AS-011/012/013 enforce) so the FE renders the toggle without re-deriving the permission
- **Data:** payload includes { isDefault, visibility, canToggleVisibility } per project; canToggleVisibility = true for the owned rows, false for the non-owned public one
...
AS-030: Each project the picker offers reports the access a new doc there would get (carve-out applied)
- **Given:** the projects payload the new-doc / move-copy picker reads, with a public project, a non-default private project, and the member's default (private-shell) project
- **When:** the picker payload is built
- **Then:** each project row carries `newDocAccess` — the derived level a new doc created there WOULD get (`deriveNewDocAccess(isDefault, visibility)`: public → workspace-shared, non-default private → restricted, DEFAULT project → workspace-shared via the carve-out) — so the FE's pre-publish access hint DISPLAYS the server-derived outcome instead of re-deriving the rule (and so the default-project carve-out can never be mis-mirrored client-side)
- **Data:** payload rows include { newDocAccess }; public → "anyone_in_workspace", non-default private → "restricted", default project → "anyone_in_workspace"
...
- **Then:** the server detects the mismatch and refuses with a STABLE machine-readable discriminator (a `reason: "visibility_boundary"` in the refusal body, distinct from any other conflict on the move endpoint) — it never silently keeps or changes access; the FE keys on that discriminator (not the HTTP status or the message text) to show the choice dialog before retrying
- **Data:** shared doc → private project, no choice → refused with reason = visibility_boundary
- **Setup:** a different conflict on the same endpoint (e.g. a slug/state conflict) carries a different reason, so the FE never mistakes it for a visibility-boundary refusal
...
- C-001: A project's `visibility` ∈ `private | public`; default project private, user-created
  public-by-default and OVERRIDABLE AT CREATE on both surfaces — web (the New Project dialog) and MCP
  (`create_project`'s param). (AS-001, AS-002, AS-003, AS-004; web create-time override → `project-visibility-fe:AS-018, AS-019`)
...
- C-009: Moving a doc into a project whose visibility differs from the doc's current access requires
  ...
  in one transaction; the server never silently changes or keeps access without the choice. The
  needs-choice refusal carries a STABLE machine-readable discriminator (`reason: "visibility_boundary"`)
  distinct from any other conflict on the move endpoint, so the consumer keys on it — not on the HTTP
  status or message text. (AS-021, AS-022, AS-023)
...
- C-011: The projects-list / picker read carries, per project, `isDefault` and `visibility` (for the
  badges), `canToggleVisibility` (server-computed: owner always, admin only on public — so the FE
  renders the toggle without re-deriving the gate), and `newDocAccess` (the derived level a new doc
  there would get, default-carve-out applied — so the pre-publish access hint displays a server-derived
  outcome). (AS-015, AS-030)
...
- `canToggleVisibility` (project) — consumed by `project-visibility-fe:AS-002/003` to decide whether to
  render the visibility toggle (the FE must NOT re-derive the owner/admin gate). Produced here by
  **AS-015** (server-computed per row). ✔ match. (Added by the /mf-challenge be-addendum — the list row
  previously carried no owner identity, so the affordance was uncomputable client-side.)
- `newDocAccess` (project) — consumed by `project-visibility-fe:AS-006` for the new-doc access hint (so
  the FE displays the server-derived level, never re-deriving the default-project carve-out). Produced
  here by **AS-030** on the picker payload. ✔ match. (Added by the be-addendum.)
- move-refusal `reason: "visibility_boundary"` — consumed by `project-visibility-fe:AS-009` to key the
  boundary alert (not the HTTP status / message). Produced here by **AS-021** / C-009. ✔ match.
  (Added by the be-addendum — the refusal previously had no discriminator distinct from other 409s.)
...
Stories=7 (= soft target). AS=30 (10 over the 20 soft target, AT the 30 hard cap).
...
- S-004 derivation + transparency: AS-016/017/018/019/020 (public / non-default-private / default-carve-out /
  MCP-loop / copy — C-007 cross-surface) + AS-029 (publish reports resulting access — C-013) + AS-030
  (newDocAccess picker preview — be-addendum producer for the FE hint) — 7 atoms.
...
At the 30 hard cap: any further FE-contract producer must go to a sibling spec, not this one.
...
| 2026-06-26 | Create-time visibility (snapshot 2026-06-26-be-create-vis): C-001 + S-001 description extended — web user-created visibility is overridable AT CREATE (New Project dialog), not just MCP, closing the create-public-then-toggle exposure window. The web create-time control AS lives in `project-visibility-fe:S-005` (this spec is at the 30-AS hard cap; the behaviour is a FE-dialog + route vertical slice). | /mf-plan |
| 2026-06-26 | BE addendum (snapshot 2026-06-26-be-addendum): /mf-challenge of `project-visibility-fe` exposed 3 FE-consumed fields the BE didn't produce. +`canToggleVisibility` on the list row (AS-015/C-011 — the row had no owner identity, affordance was uncomputable client-side); +`reason:"visibility_boundary"` discriminator on the move refusal (AS-021/C-009 — was a generic 409 indistinguishable from other conflicts); +`newDocAccess` preview field (new AS-030/C-011 — so the FE hint displays the server-derived level instead of re-mirroring the carve-out). AS 29→30. These 3 are a small BE BUILD owed (code not yet written) before the FE seam tests pass. | /mf-challenge |
