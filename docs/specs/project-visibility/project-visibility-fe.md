...
### S-005: Create a project with a chosen visibility (P1)

**Description:** As a workspace member creating a project on the web, I can choose Public or Private
right in the New Project dialog (defaulting to Public) — so an intended-private project is private the
moment it exists, without a create-public-then-toggle round-trip or the window where it is briefly
public. This is the web counterpart of MCP `create_project`'s visibility param.
**Source:** user request 2026-06-26 (web create has no visibility choice while MCP does — asymmetry + exposure window); `project-visibility:C-001` (create-time override on both surfaces).
**Applies Constraints:** C-001

**Execution:**
- `depends_on:` none
- `parallel_safe:` false
- `files:` `apps/web/src/features/docs/components/` (the New Project dialog — add the Public/Private control), `apps/web/src/features/docs/services/client.ts` (send `visibility` on create), `apps/backend/src/routes/projects.ts` (accept `visibility` in the create body and pass it to `createProject`, which already takes the optional param), `apps/backend/src/workspace/projects.ts`
- `autonomous:` true
- `verify:` open New Project, leave the control at Public → created public; choose Private → the project exists private immediately (never appears public to other members), no separate toggle.

**Acceptance Scenarios:**

AS-018: Creating a project at the default leaves it public
- **Given:** the New Project dialog with the visibility control at its default
- **When:** the member names the project and creates it without changing the control
- **Then:** the project is created public — the prior default behaviour is preserved
- **Data:** create "Vantage" at the default control → public

AS-019: Choosing Private at create makes the project private immediately
- **Given:** the New Project dialog with the visibility control set to Private
- **When:** the member creates the project
- **Then:** the project is created private from the moment it exists — it never appears public to other workspace members, and no follow-up toggle is needed
- **Data:** create "Vault" with the control set to Private → private on creation
...
- C-001: All visibility UI (badge, access hint, boundary alert, suppressed name, publish notice)
  ...
  notice reads `project`/`access`; the create-time control COLLECTS an explicit user choice (it does
  not derive one). No client-side replica of any server rule. (AS-001, AS-003, AS-008, AS-016, AS-019)
...
projects payload — conditionally-mounted routes cast to `unknown`); the seam AS (the AS-006
contract + the AS-015 behavioural integration) cover presence/value + the one stateful flow.
...
| 2026-06-26 | Create-time visibility (snapshot 2026-06-26-fe-create-vis): +S-005 vertical-slice story (the New Project dialog gains a Public/Private control, default Public; the create route/schema honors it — the service `createProject` already takes the optional param) + AS-018/019. Closes the create-public-then-toggle exposure window and the web↔MCP asymmetry. Lives here (not in `project-visibility`, which is at the 30-AS hard cap) and is a FE-dialog + route vertical slice. Stories 4→5, AS 17→19. | /mf-plan |
