...
**Date:** 2026-06-07
**Ref:** --
**Reason:** M6 — C-008/C-009 constraint meaning generalized (SMTP-only → email provider: SMTP or Resend HTTP API); new AS-012 (mail delivered via configured provider)
...
AS-011: A runtime SMTP failure does not get stuck permanently [harden H6]
- **Given:** SMTP is configured OK at boot but the provider errors/rate-limits when sending
...
- **Data:** SMTP returns 5xx at runtime
...
- C-008: SMTP is mandatory — the app does not start if SMTP config is missing (like APP_SECRET);
  therefore email verify always works, no no-verify mode. (AS-001)
- C-009 [harden H6]: SMTP mandatory at boot is DIFFERENT from being able to send at runtime — every
  outbound mail enqueues + retries + dead-letters + surfaces a "send failed" status to the operator;
  pending invites carry an accept link (in-app/shareable) that works regardless of whether the
  email arrives. (AS-011)
...
- GAP-001 (status: resolved → C-008): SMTP is **mandatory** — the app does not start if
  SMTP is not configured. So email verify always sends; no degrade mode.
  (Decided 2026-06-07; reverses explore's "SMTP optional" — `self-host` must update:
  SMTP is mandatory boot config like APP_SECRET.)
