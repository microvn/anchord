...
**Date:** 2026-06-07
**Ref:** --
**Reason:** M4/M5 — AS-004 + C-002 generalized SMTP-required → email-provider-required (SMTP or Resend HTTP API), mirroring auth C-008
...
- No new business entity. Infrastructure:
  ...
  - Config via env: `APP_SECRET`, `DATABASE_URL`, `SMTP_*`, OAuth `GITHUB_*`/`GOOGLE_*`
...
- `autonomous:` true
- `verify:` drop APP_SECRET → app won't start, log states it clearly; drop SMTP → same.
...
AS-004: Missing SMTP → refuse to start
- **Given:** `.env` has not configured SMTP
- **When:** the app boots
- **Then:** the app won't start; the log clearly states SMTP is required (consistent with auth C-008)
- **Data:** empty SMTP_*
...
- C-002: Required config (APP_SECRET ≥16, DATABASE_URL, SMTP_*) validated at boot;
  missing/invalid → refuse to start with a clear log. (AS-003, AS-004)
...
- **SMTP config (required)** — produced by self-host (env validate, C-002).
  Consumed by `auth` (verify/invite), `workspace-project` (notify). ✔ self-host guarantees
  SMTP is always present, so those clusters don't need to degrade.
...
- **SMTP REQUIRED (reversing the explore "optional + degrade"):** the app won't start if
  SMTP is missing → drop all degrade logic; email verify/invite/notify always work. Consistent
  with `auth` C-008.
...
- **SMTP runtime (from /mf-challenge H6):** required at boot is DIFFERENT from being able-to-send at
  runtime; outbound mail enqueue + retry + dead-letter + an error state for the operator (details
  in `auth` C-009). Boot still fails fast if SMTP config is missing.
