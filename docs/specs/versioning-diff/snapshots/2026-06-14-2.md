# Snapshot: versioning-diff
**Date:** 2026-06-14
**Ref:** --
**Reason:** M5 (AS-003 Then changed — history list order pinned to newest-first / current-first)
...
- **History entry (read shape)**: each version-history item exposes the publisher as
  `{ id, name }` — the display name resolved from `published_by` (the author id), with a fallback
  label when the author is unknown/unresolved. The raw author id is never the only thing shown. (C-006)
- **Diff rendered reference**: the compare result references each compared version's rendered
  content by a **per-version content reference** — one per version — that resolves to THAT version's
  sandboxed content (the same per-version content surface the viewer loads), not the current
  version's. Holds for both text and image docs. (C-007)
...
- `depends_on:` S-001
- `parallel_safe:` false
- `files:` `apps/backend/src/services/version.ts` (`listVersionHistory` — resolve `publishedBy` id → name), `apps/backend/src/routes/versions.ts` (`GET …/versions`)
- `autonomous:` true
- `verify:` list history for a doc with versions by a named user → each entry carries `publishedBy.name`; a version with no resolvable author → fallback label, not a raw id.
...
AS-011: Each history entry shows the publisher's resolved display name
- **Given:** version 3 was published by a user whose name is "Mara Lindqvist"
- **When:** the user opens "Version history"
- **Then:** the v3 entry shows the publisher's display name "Mara Lindqvist" (resolved from the
  author id), not an opaque author id
- **Data:** v3 published_by = Mara's user id; Mara's name = "Mara Lindqvist"

AS-012: A version with an unknown author shows a fallback label
- **Given:** a version whose author cannot be resolved to a name (no recorded author, or the author
  no longer resolves)
- **When:** the user opens "Version history"
- **Then:** that entry shows a fallback publisher label (e.g. "Unknown"), never a blank field or the
  raw author id
- **Data:** a version with no resolvable author
...
- `depends_on:` S-001
- `parallel_safe:` false
- `files:` `apps/backend/src/services/diff.ts` (`compareVersions` — `renderPair` per-version content reference resolving to the served `/v/:versionId` surface), `apps/backend/src/routes/versions.ts` (`GET …/diff`); rendered side-by-side reuses the viewer per-version content route
- `autonomous:` true
- `verify:` diff v2↔v3 → `renderPair` has two references; each loads the matching version's content (v2's ≠ v3's), not the current version.
...
AS-013: The compare result references each version's own rendered content
- **Given:** doc has version 2 and version 3 with different content
- **When:** the user compares "v2 ↔ v3"
- **Then:** the result carries one rendered-content reference per compared version, and each
  reference loads THAT version's content (v2's reference renders v2, v3's renders v3) — not the
  current version's; so the side-by-side shows the two distinct versions
- **Data:** v2 ≠ v3; the two references resolve to v2 and v3 content respectively
...
- C-006: Every version-history entry carries the publisher's resolved display name (resolved from
  the author id), with a fallback label when the author is unknown/unresolved — never an opaque
  author id alone and never a blank publisher. (AS-011, AS-012)
- C-007: The compare result references each compared version's rendered content by a per-version
  content reference that resolves to THAT version's sandboxed content (the per-version content
  surface the viewer loads), for both text and image docs — never the current version's content. (AS-013, AS-006, AS-008)
...
- **`publishedBy: { id, name }`** on the history read — produced here by S-002 (AS-011/012, C-006)
  on the `GET …/versions` list surface (served on every history fetch). Consumed by
  `versioning-diff-ui:S-001` to show the publisher name + avatar on the timeline. ✔ producer AS now
  delivers the resolved name on the list surface (resolves the FE-owed publisher-name gap).
- **`renderPair` per-version content reference** on the diff read — produced here by S-004
  (AS-013, C-007) on the `GET …/diff` response, one reference per compared version resolving to that
  version's sandboxed content. Consumed by `versioning-diff-ui:S-003/S-004` as the Rendered side-by-side
  + image-pair iframe sources. ✔ producer AS now pins each reference to its own version (resolves the
  FE-owed per-version render gap; the existing per-version content surface serves it).
...
All paths are **workspace-scoped** (`/api/w/:workspaceId/…`) like every other app route —
matches the actual code (`apps/backend/src/routes/versions.ts`). `→` = serves which AS.

| Method · Path | Serves | Auth | Request | Success | Errors |
|---|---|---|---|---|---|
| `POST /api/w/:ws/docs/:slug/versions` | S-001 (AS-001) | session (editor) | `{ content, contentHash? }` (Zod) | 201 `{ version, previousVersion }` | 404 NOT_FOUND (no-access/missing, C-006), 403 FORBIDDEN (not editor) |
| `PATCH /api/w/:ws/docs/:slug` | S-001 (AS-002 title-only, no version) | session (editor) | `{ title }` (Zod) | 200 `{ slug, title }` | 404, 403 |
| `GET /api/w/:ws/docs/:slug/versions` | S-002 (AS-003/011/012 history) | session (viewer+) | pagination query (api-core C-008) | 200 `{ items: [{ version, createdAt, publishedBy: { id, name }, isCurrent }], pagination }` (C-006: `publishedBy.name` resolved, fallback when unknown) | 404 |
| `POST /api/w/:ws/docs/:slug/versions/:n/restore` | S-003 (AS-004) | session (editor) | — | 201 `{ version, previousVersion }` | 404 (doc or version n missing), 403 |
| `GET /api/w/:ws/docs/:slug/diff?from=&to=` | S-004 (AS-006/007/008/013) | session (viewer+) | query `from`,`to` (Zod) | 200 `{ mode, identical?, changeCount?, lines?, renderPair }` — `renderPair` = one per-version content reference per compared version, each resolving to that version's sandboxed content (C-007) | 404, 400 VALIDATION_ERROR (bad version refs) |
...
| 2026-06-14 | Major (M6): +C-006 (history entry carries publisher's resolved name + fallback) → +AS-011/012; +C-007 (diff references each version's own rendered content per-version) → +AS-013; Data Model + API table updated (publishedBy→{id,name}, renderPair per-version, paths → workspace-scoped); Linked Fields pin both for `versioning-diff-ui`. Resolves FE GAP-001/002/003 producer-side. Snapshot 2026-06-14.md | -- |
