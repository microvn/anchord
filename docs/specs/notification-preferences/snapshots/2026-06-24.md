...
**Date:** 2026-06-24
**Ref:** --
**Reason:** M5/M6 — doc-share `invited` reverted to in-app-only (was in-app + email); the transactional invite email is the only email, the notification channel never emails for `invited`.
...
- **Supported-channel matrix per event (the default-on set)** — defines which channels each
  event offers and which are on by default. This is where the doc-share `invited` event gains
  email by default (it is in-app-only today):
  ...
  | invited (doc shared with you) | on | **on** | upgraded from in-app-only |
...
AS-006: Documents shared with you sends an email by default
- **Given:** a user with default (unchanged) preferences
- **When:** a document is shared with them
- **Then:** they receive both an in-app row and an email (the doc-shared default now includes email)
...
every event and email on for the high-signal personal events; the doc-share `invited` event
defaults to in-app + email, and `workspace_member_joined` email is the one channel OFF by
default (opt-in). (AS-001, AS-006)
...
- `NotificationsSettingsSection` *(registered over the reserved `notifications` slot, clearing its coming-soon stub)*
  - `NotificationPrefGroup: Comments & feedback`
    - `NotificationPrefRow` per event: label + description + `In-app` toggle + `Email` toggle *(detached row: in-app locked on, no email toggle)*
