...
**Date:** 2026-06-24
**Ref:** prototype-fidelity (product owner)
**Reason:** M4, M6 — C-007 REVERSED: Your-actions no longer reuses the workspace-activity feed components; it is built from the personal `.me-*` family to match the `personal.jsx` prototype (per-day cards, type-toned node icons, no actor avatar/name, no event counts). S-001 flow + AS-005 + Linked Fields changed.
...
AS-005: Opening an action shows its detail and links to the doc (reused detail)
- **Given:** a share action on a doc
- **When:** I open the row
- **Then:** the detail (the SAME component `workspace-activity` provides — not a 2b rebuild) shows the workspace, doc, when, and the action-specific detail (e.g. access · role for a share), with an "Open in doc" link to the target
...
C-007: The Your-actions feed REUSES `workspace-activity`'s feed/row/chips/detail components and read repo (filtered to `actorUserId` instead of `workspaceId`) — it does NOT re-implement the feed rendering. (AS-005; Linked Fields)
...
- **`workspace-activity` feed/row/chips/detail components** — consumed by `your-activity-actions:S-001` (AS-005, C-007) by rendering `/api/me/activity` data through them. Produced by `workspace-activity` (its Export contract + Linked Fields, locked 2026-06-23) as composable/rows-as-props. ✔ locked (GAP-002 resolved).
