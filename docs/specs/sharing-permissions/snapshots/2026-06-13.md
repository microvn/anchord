...
**Date:** 2026-06-13
**Ref:** --
**Reason:** M1 (new story S-007 change/remove a doc member) + M6 (new C-017)
...
- **Then:** returns the general-access level, the associated link role, guest-commenting on/off,
  editors-can-share on/off, the list of invited people (each with email/name, role, and
  active|pending status), and the link controls (expiry, view limit, view count, and WHETHER a
  password is set)
- **Data:** anyone-with-link/commenter, guest on, 1 pending + 1 active invite, password link
...
|---|---|---|---|---|---|
| `GET /api/w/:workspaceId/docs/:slug/share` | S-006 (AS-025/026/027) | session: owner, or editor when `editors_can_share` (C-007) | — | 200 `{ level, role, guestCommenting, editorsCanShare, people[], link{ hasPassword, expiresAt, viewLimit, viewCount, url } }` | 403 FORBIDDEN (cannot-manage AS-027), 404 |
...
Stories=6 (under the target of 7). AS=27 (target 20, in the G7 overage range ≤30).
