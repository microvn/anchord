...
**Date:** 2026-06-20
**Ref:** --
**Reason:** M2+M6 — drop the guest-commenting sub-toggle (Google-Docs model): anon-write gated by link role ≥ commenter, not a separate toggle. Removes AS-003 + C-003 + the `guestCommenting` field from the share contract.
