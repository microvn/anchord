...
**Date:** 2026-06-08
**Ref:** --
**Reason:** M6/M5 — manage-sharing model changed owner-only → Google-Docs (owner always + editor when editors_can_share); C-007 rewritten, AS-014 repurposed, +editors_can_share
...
AS-014: Only the owner manages sharing
- **Given:** a user with the editor role (not owner)
- **When:** they open the Share box to change general-access/invite people
- **Then:** not allowed (v0: only the owner manages sharing)
- **Data:** editor tries to change access
...
- C-007: v0: only the owner manages sharing (change access, invite, link controls). (AS-014)
...
|---|---|---|---|---|---|
| `PUT /api/docs/:slug/access` | S-001 (AS-001/002/003/018) | session (owner, C-007) | `{ level, role, guestCommenting? }` (Zod) | 200 `{ level, role, guestCommenting }` | 400 VALIDATION_ERROR (invalid role AS-018; guest-on-restricted AS-003), 403 FORBIDDEN (editor, AS-014), 404 |
| `POST /api/docs/:slug/invites` | S-003 (AS-007/008) | session (owner) | `{ email, role, message? }` (Zod) | 201 `{ status }` active\|pending | 400, 403, 404 |
| `PUT /api/docs/:slug/link` | S-004 (AS-009/010/011/016/017/019/020/021) | session (owner) | `{ password?, expiresAt?, viewLimit? }` (Zod) | 200 `{ link controls }` | 400, 403, 404 |
...
- **v0: only the owner manages sharing** (editors don't invite people).
...
Stories=5 (under the target of 7). AS=21 (target 20, in the G7 overage range ≤30).

The single AS over the soft target comes from G1 splits documenting build-time S3 guards, each its
own atom (no AS gộp):
...
- S-001: AS-018 (invalid-role rejection) — 1 validation atom.

No bloat — each AS traces to one stated guard.
