...
- **Then:** returns the general-access level, the associated link role, guest-commenting on/off,
  ...
  member actions, S-007), and the link controls (expiry, view limit, view count, and WHETHER a
  password is set)
...
|---|---|---|---|---|---|
| `GET /api/w/:workspaceId/docs/:slug/share` | S-006 (AS-025/026/027) | session: owner, or editor when `editors_can_share` (C-007) | — | 200 `{ level, role, guestCommenting, editorsCanShare, people[]{ id, email, name?, role, status }, link{ hasPassword, expiresAt, viewLimit, viewCount, url } }` | 403 FORBIDDEN (cannot-manage AS-027), 404 |
...
| `POST /api/w/:workspaceId/docs/:slug/invites` | S-003 (AS-007/008) | session: owner, or editor when `editors_can_share` (C-007) | `{ email, role, message? }` (Zod) | 201 `{ status }` active\|pending | 400, 403, 404 |
...
- **share-state read** (`GET …/share` → `{ level, role, guestCommenting, editorsCanShare,
  people[], link{ hasPassword, expiresAt, viewLimit, viewCount, url } }`) — produced by S-006
  (AS-025). Consumed by `sharing-permissions-ui` to PREFILL the ShareDialog on open. ✔ password
