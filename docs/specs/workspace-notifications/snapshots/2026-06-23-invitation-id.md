...
**Date:** 2026-06-23
**Ref:** invitationId (your-activity-inbox:GAP-001)
**Reason:** M4 (workspace_invited refId semantics: workspace_id → invitation_id), M1 (+AS-010), M6 (+Linked Field producer obligation).
...
- **`notifications.refId`** — for these types holds the `workspace_id`. NOTE the existing
  read surface (`notify/read-repo.ts listForUser`) enriches every row by joining
  `refId → annotations → docs`; a workspaceId matches no annotation, so doc-based
  enrichment returns null for these rows. The read path must NOT live-join `workspaces`
  (that would leak the workspace's CURRENT name to a member who was since removed — F1).
