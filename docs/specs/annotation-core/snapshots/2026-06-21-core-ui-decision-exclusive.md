...
**Date:** 2026-06-21
**Ref:** --
**Reason:** M4 (AS-026/029/030 flow) + M6 (C-009/C-010/C-011) — Decision axis vacuous-pass → EXCLUSIVE-when-narrowed; filter popover: drop duplicate "Reset all", per-axis All shortcut becomes an All/None toggle
...
AS-026: With the Status or Type axis fully deselected the rail shows a distinct no-match state
...
- **Data:** Type={} on the 23-annotation doc (the Decision axis behaves differently — see AS-030)
...
AS-029: Deselecting a Decision facet hides those suggestions and dims their marks, leaving non-suggestions
- **Given:** the Filter popover open with all facets selected, on a doc with both accepted suggestions and plain comments
- **When:** I deselect the Accepted decision facet
- **Then:** accepted suggestions leave the rail list and their in-text highlights dim, while every comment and label stays in the rail with its highlight unchanged (a comment / label has no decision, so the Decision axis never hides it); other decisions and all statuses/types stay
- **Data:** deselect Accepted; the doc has 1 accepted markup + 18 comments → the 18 comments remain, the accepted markup goes

AS-030: Deselecting every Decision facet hides all suggestions but is NOT the no-match state
...
- **Then:** every suggestion (Markup / Redline) leaves the rail and its mark dims, but the rail still lists the comments and labels (they pass the Decision axis vacuously) — this is NOT the no-match state, unlike emptying the Status or Type axis (AS-026)
- **Data:** Decision={} on the 23-annotation doc → the 4 suggestions hidden, the 19 comments/labels remain
...
- C-009: The rail filters on THREE independent (orthogonal) axes — Status {Open, Resolved}, Type
  ...
  PARTIAL axis — it applies ONLY to suggestions (Redline / Markup); a Comment / Label has no decision
  and PASSES the Decision axis vacuously (it is never hidden by it, the standard faceted-search rule for
  an attribute an item lacks). A thread is SHOWN iff its status facet is selected AND its type facet is
  selected AND (it is a non-suggestion, OR its decision facet is selected) — OR within an axis, AND
  across axes. The DEFAULT selection is the baseline: Type and Decision default to all-selected, but
  Status defaults to Open ONLY (Resolved OFF) — so resolved threads are hidden from the rail (and their
  marks dimmed) until the reviewer enables Resolved. Toggling a facet OFF hides its matching annotations
  from the rail and dims their in-text highlights; re-selecting restores both. An `isOrphaned`/detached
  annotation is counted into its facets and still appears in the separate detached section, which renders
  regardless of filter state (C-004). When the STATUS or the TYPE axis has no facet selected, the rail
  shows a no-match state distinct from the empty-doc state; emptying the DECISION axis is NOT no-match —
  it hides every suggestion but leaves the comments/labels (they pass vacuously). Acting on a
  filtered-out highlight/thread re-activates whichever of its facets are off across the (up to three)
  axes that apply to it, then focuses it. (AS-022, AS-023, AS-024, AS-026, AS-028, AS-029, AS-030)
- C-010: Facet counts are DYNAMIC — each facet's count is the number of annotations it would match
  ...
  within an axis never zeroes that axis's own siblings). For the Status and Type axes, a non-suggestion
  satisfies the Decision-axis term vacuously, so it still counts. The Decision-facet counts are over
  SUGGESTIONS only (a comment/label has no decision, so it contributes to no Decision facet) — hence the
  Decision group sums to the number of visible suggestions, not the whole doc; that asymmetry with the
  Status/Type groups is by design, not a bug. A facet that would yield zero shows 0; it is never hidden.
  (With every axis fully selected the Status/Type counts equal the whole-doc per-facet totals and the
  Decision counts equal the per-status totals over suggestions.) (AS-025, AS-029)
- C-011: The filter is presented in a popover opened from the rail's Filter control (a bottom-sheet on
  ...
  LIVE (the rail + marks update immediately, no explicit Apply); Reset returns to the DEFAULT (Status =
  Open only, Type all-selected, Decision all-selected) — NOT to all-selected. The "active" signal is
  measured against the DEFAULT, not against all-selected: while the selection differs from the default
  the rail header reads "showing X of N" and the Filter control reads active; at the default it reads
  inactive and the header shows the default total (the open count). Enabling Resolved (or any deviation)
  flips the control to active. (AS-022, AS-027)
  ...
  - `ViewerBody` *(kind-conditional per C-006 — markdown: 3-pane outline·content·annotations, outline collapsible; html/image: 2-pane full-width content·annotations, no outline. Collapses to drawers <1200 TOC / <600 rail)*
    ...
    - `AnnotationsRail`
      - `RailHeader`: total + "showing X of N" *(when narrowed, C-011)* · `FilterControl` *(active when narrowed)* · empty state · no-match state
        - `FilterPopover` *(C-011; bottom-sheet on mobile)*: `FacetGroup` Status *(Open · Resolved)* + `FacetGroup` Type *(Markup · Comment · Redline · Label — tool hue + icon)* + `FacetGroup` Decision *(Pending · Accepted · Rejected · Stale — reuse the ThreadCard lifecycle-pill tones: pending neutral · accepted success · rejected error · stale muted; suggestions only)* → `Facet` *(icon + dynamic count + selected state, C-010)* · Reset
...
- S-007 three-axis filter: 9 AS for 9 atoms — popover lists all three axes + counts (AS-022), Type-facet
  filter hides+dims (AS-023), AND-across combine (AS-024), dynamic counts (AS-025), Status/Type axis-empty
  no-match (AS-026), header "showing X/N" + Reset (AS-027), act-on-filtered-out re-activates the off
  facets (AS-028), Decision-facet hides suggestions only / non-suggestions pass vacuously (AS-029),
  Decision-axis-empty hides all suggestions but is NOT no-match (AS-030). Each is a distinct assertion.
  The 2 added atoms (AS-029, AS-030) carry the PARTIAL-axis behaviour — the Decision axis applies only to
  suggestions, so its single-facet effect and its empty-axis effect both differ from the Status/Type
  axes; neither is a variant of an existing AS.
