...
**Date:** 2026-06-11
**Ref:** --
**Reason:** M6 — new constraint C-014 (sandbox-bridge CSP + realm-isolation decision); resolve GAP-004 (bridge built); add GAP-005 (nested-iframe hardening, deferred)
...
- GAP-004 (status: deferred): the effort to adapt Plannotator's `HtmlBlock`/`useHtmlAnnotation`
  (assumes srcdoc) to src+content-route — measured at build time. Source: "Reuse HtmlBlock
  … needs adapting … how much effort".
