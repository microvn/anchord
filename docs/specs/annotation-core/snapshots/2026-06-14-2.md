...
**Date:** 2026-06-14
**Ref:** --
**Reason:** M4 (AS-018 When disambiguated to the top-bar toggle) + new AS-019 (in-pane outline collapse chevron) + UI Notes TocSidebar control
...
AS-018: Collapsing the outline gives the content more room
- **Given:** the viewer is open on a Markdown doc with the outline shown
- **When:** I collapse the outline (outline-toggle)
- **Then:** the outline pane hides and the content area reflows wider; toggling again restores it
...
- C-006: The viewer layout is conditional on doc kind. kind=markdown → 3-pane: a collapsible outline ·
  ...
  (the HTML sandbox is cross-origin, images have no headings — GAP-004). (AS-001, AS-002, AS-003, AS-018)
...
- `ViewerScreen` `[N]` *(React route `/d/:slug`; replaces the server-rendered `/d` page — direction B)*
  ...
  - `ViewerBody` *(kind-conditional per C-006 — markdown: 3-pane outline·content·annotations, outline collapsible; html/image: 2-pane full-width content·annotations, no outline. Collapses to drawers <1200 TOC / <600 rail)*
    - `TocSidebar` *(kind=markdown only)*: `TocSearch` · `TocGroup` → `TocItem` *(scroll-spy active; `PriorityBadge` if spec)*
...
Stories=6, AS=18 — both under the soft target. This is sub-spec 1 of a 3-way by-flow split of
