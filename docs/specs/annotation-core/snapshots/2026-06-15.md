...
**Date:** 2026-06-15
**Ref:** --
**Reason:** M6 — read-contract constraint extended (annotation list read serves suggestion payload + suggestion_status); + AS-030
...
- **annotations** (anchored to the **doc**, not to a version): `id`, `doc_id`, `type`
  ...
  `suggestion_status` (nullable — pending | accepted | rejected | stale), `label` (nullable — a
...
- **`suggestion.kind=delete` create + decide** — produced HERE by S-006 (create accepts `to`-omitted =
  delete; decide is kind-agnostic, AS-015/022). Consumed by `annotation-core-ui-types-modes`:S-002 (Redline).
  ✔ delete is symmetric to replace (verified — see Clarifications).
...
| `GET /api/w/:workspaceId/docs/:slug/annotations` | S-001 (AS-021 read-authz), S-009 (AS-027 serves `label`) | session (viewer+) | pagination query | 200 `{ items, pagination }` (each item carries `label` when set) | 404 (no-access → indistinguishable, C-006) |
...
Stories=9 (target 7, within G7 overage ≤10). AS=29 (target 20, within G7 overage ≤30).
