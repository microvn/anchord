...
### S-006: A proposal's birth status follows the creator's edit authority (P1)

**Description:** A creator who can EDIT the doc (owner/editor) has the authority to make the change a
proposal asks for, so their OWN proposal is created already **accepted** — not pending. There is no
review limbo and nothing to self-decide (you can't self-approve a pending proposal anyway — S-003). A
creator who CANNOT edit (commenter, guest) creates a **pending** proposal that awaits an owner
decision, exactly as before. At create the proposal's `from` span is the just-selected current text,
so it matches by construction — no stale risk in being born accepted.
**Source:** PO 2026-06-17 ("owner redline đúng document của họ thì nên accept ngay thay vì pending"); scope = owner + editor (creators with `can(role,"edit")`).

**Execution:**
- `depends_on:` S-001
- `parallel_safe:` false
- `files:` `apps/backend/src/annotation/suggestion.ts` (createSuggestion gates the born `status` on `can(sessionRole,"edit")`), `apps/web/src/features/viewer/hooks/use-compose.ts` (the optimistic + no-refetch reconciled redline mirrors the born status so it never flashes Pending), `apps/web/src/features/viewer/components/viewer-screen.tsx` (passes the edit-authority flag from `effectiveRole`)
- `autonomous:` true
- `verify:` as an owner, create a redline → it shows Accepted immediately (not Pending); as a commenter, create one → it shows Pending.

**Acceptance Scenarios:**

AS-017: An owner's own proposal is born accepted
- **Given:** the doc owner selects text and creates a redline (delete) on their own doc
- **When:** the proposal is created
- **Then:** its lifecycle status is **accepted** at birth (not pending); no owner decision is awaited
- **Data:** creator role = owner

AS-018: An editor's own proposal is born accepted
- **Given:** an editor (can edit the doc) creates a suggestion (redline or replace)
- **When:** the proposal is created
- **Then:** its lifecycle status is **accepted** at birth — the editor's edit authority is the same authority a decide would confer
- **Data:** creator role = editor; both delete and replace kinds

AS-019: A commenter's proposal is born pending
- **Given:** a commenter (cannot edit the doc) creates a redline
- **When:** the proposal is created
- **Then:** its lifecycle status is **pending** — it awaits an owner decision, unchanged from before
- **Data:** creator role = commenter (a guest, also no edit authority, is likewise born pending)
...
- C-004: No self-approve — a proposal whose creator (`author_id`) equals the acting user is treated as
  ...
  creator (guest) is never a self and is always owner-decidable. Requires C-005. With C-008 in force, an
  own proposal that is PENDING arises only via a REOPEN of the creator's own accepted proposal (it is no
  longer created pending) — the no-self-approve treatment then applies to that reopened-pending state. (AS-005, AS-006, AS-007)
...
- C-008: Birth status by authority — at create, a proposal's `suggestion_status` is **accepted** when
  the creator `can(role,"edit")` (owner/editor), else **pending** (commenter/guest). The creator's edit
  authority is the same authority an Accept would confer, so their proposal needs no review; the `from`
  span is the just-selected current text at create, so an accepted-at-birth proposal is never stale.
  This sets only the suggestion lifecycle — the annotation's own `status` stays unresolved (identical to
  the state an owner-accept of another's proposal produces). Complements C-004 (own-pending now only via
  reopen). (AS-017, AS-018, AS-019)
...
| 2026-06-17 | Mode C (Major, M1+M6) — added S-006 (AS-017/018/019) + C-008: a creator who can edit (owner/editor) born-accepts their OWN proposal at create; commenter/guest stay pending. Amended C-004 (own-pending now arises only via reopen). Reflects the shipped backend createSuggestion auto-accept + the FE optimistic mirror. Snapshot 2026-06-17-actions-born-accept.md. | PO 2026-06-17 |
