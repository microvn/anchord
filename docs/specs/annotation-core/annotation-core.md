...
|---|---|---|---|---|---|
| `POST /api/w/:workspaceId/docs/:slug/annotations` | S-001 (AS-001/002/003), S-002 (AS-005/006), S-009 (AS-027/028/029 labeled) | session/guest with comment role | `{ type, anchor, label? }` (Zod; text/image-region; `label` validated ∈ preset set, C-015; rejected with a `suggestion` payload) | 201 `{ annotationId }` | 400 VALIDATION_ERROR (empty selection AS-004; unknown label AS-028; label+suggestion AS-029), 403 FORBIDDEN (viewer, AS-020 forged role), 404 (no-access doc, AS-021/C-006) |
