...
### S-001: Determine own-vs-others from the durable creator id (internal; real name+avatar shown) (P0)

**Description:** As a reviewer, every rail item shows its author's REAL name + avatar (an authenticated
user has an identity — do NOT relabel my own as "You"). Separately, the app determines own-vs-others from
the annotation's durable creator id (`authorId`, served on the read — NOT inferred from the first comment)
and keeps that purely INTERNAL: it drives the no-self-approve gate + the delete-own affordance, with no
visible own/other marker on the rail. A guest-created annotation (null creator id) is never internally own.
**Source:** conversation 2026-06-16/17 (PO: own is internal, show real name+avatar, drop "You");
...
- `autonomous:` true
- `verify:` open a doc with one of your annotations + one by another member → BOTH show the real author name+avatar the same way (no "You"); internally your own is recognised (Delete shows on yours; no Accept/Reject on your own proposal).

**Acceptance Scenarios:**

AS-001: A member's own annotation shows their real name + avatar (no "You" relabel)
- **Given:** I am signed in (e.g. as "Demo User") and viewing a doc with an annotation whose creator id equals my user id
- **When:** the rail renders
- **Then:** the item shows my REAL name ("Demo User") + avatar — the SAME way it shows any other author —
  NOT a "You" relabel; the own-vs-others determination (creator id === my user id) is recorded INTERNALLY
  (it drives the no-self-approve + delete-own gates) and has NO visible marker on the rail item
- **Data:** annotation `authorId` = my user id

AS-002: Another member's annotation shows that member's name + avatar
- **Given:** a doc with an annotation whose creator id is a different member
- **When:** the rail renders
- **Then:** the item shows that member's name + avatar (the same treatment as my own — there is no
  visible own/other distinction); internally it is not-own
- **Data:** annotation `authorId` = another member's id

AS-003: A guest-created annotation shows the guest's name, never treated as own
- **Given:** an annotation created by a guest (no durable creator id)
- **When:** the rail renders for any signed-in user
- **Then:** the item shows the guest's name (+ the guest marker) and is never internally own (a null
  creator id matches no one — the gates never treat it as mine)
...
AS-020: My own pending proposal never flashes Accept/Reject while the session is still resolving
- **Given:** a signed-in owner opening a doc with a pending proposal they authored, before the session has resolved (the current user id is not yet known)
- **When:** the rail item renders during that window
- **Then:** Accept/Reject is NOT shown (the owner-decide affordance is withheld until the user id is known,
  so it never flashes on my own proposal before correcting to Resolve)
- **Data:** session still loading; a redline `authorId` = my user id
...
- C-001: The rail ALWAYS shows an annotation's real author name + avatar — own and others are displayed
  identically (no "You" relabel, no visible own marker), because an authenticated user has an identity.
  Own-vs-others is derived from the durable creator id (`authorId`, NOT the root-comment author; a null
  creator id (guest) matches no signed-in user) and kept INTERNAL — it only drives the no-self-approve
  (C-003) + delete-own (C-004) gates. (AS-001, AS-002, AS-003)
...
- C-003: No self-approve in the UI — when a Proposal's creator id equals the current user, Accept/Reject are
  ...
  user id and mirrors the server gate (`annotation-actions`:S-003). The owner-decide affordance is WITHHELD
  until the current user is known (the session has resolved): while signing-in is still loading, the user id
  is not yet available, so a signed-in owner's OWN pending proposal must NOT flash Accept/Reject before
  correcting to Resolve. (AS-008, AS-020)
...
- DEV DATA: annotations created before `annotations.author_id` existed have a null `authorId`, so the
  internal own-vs-others determination treats them as not-mine — the delete-own + no-self-approve gates
  won't recognise my OWN old annotations (e.g. an owner's old redline would still offer Accept/Reject).
  Display is unaffected (always the real comment-author name). A dev backfill (`author_id` ← the root
  comment's `author_id` where null) realigns the gates with the historical data; not a code change here.
...
| Date | Change | Ref |
|------|--------|-----|
| 2026-06-17 | Major (M5+M6): own-vs-others is now INTERNAL only (drives the no-self-approve + delete-own gates) — the rail ALWAYS shows the real author name+avatar, NO "You" relabel / no visible own marker (PO: an authenticated user has an identity; "You" hid it + split the same person into "You"/their-name on old null-`authorId` rows). AS-001/AS-002/AS-003 Then + C-001 reworked; +AS-020 + C-003: the owner-decide affordance is withheld until the session resolves (fix the Accept/Reject flash on my own proposal during sign-in load). System Impact notes the dev `author_id` backfill for old rows. Snapshot 2026-06-17-actions-ui-own-display.md | -- |
