...
AS-001: A Markdown doc renders in the app theme
- **Given:** a published Markdown doc I can access
- **When:** I open `/d/:slug`
- **Then:** its content renders styled in the app theme (not in an iframe), inside the 3-pane viewer
- **Data:** a doc with headings + a bullet list

AS-002: An HTML doc renders in an isolated sandbox
- **Given:** a published HTML doc I can access
- **When:** I open `/d/:slug`
- **Then:** the doc renders inside a sandboxed iframe (its own styles preserved, isolated origin); the app chrome does not restyle it
- **Data:** an HTML doc with its own CSS

AS-003: An image doc renders with zoom
- **Given:** a published image doc I can access
- **When:** I open `/d/:slug` and use the zoom control
- **Then:** the image displays and zooms in/out
- **Data:** a PNG doc
...
### S-002: Navigate the doc with the outline (P1)

**Description:** As a reader, I use a collapsible outline (TOC) to jump between sections; the active
section highlights as I scroll.
**Source:** annotation-core UI Notes (`TocSidebar`/`TocSearch`/scroll-spy). Prototype: `viewer.jsx` TocSidebar.
...
- **Given:** the viewer is open on a doc with several sections
- **When:** I click an outline entry
- **Then:** the doc scrolls to that section
- **Data:** a doc with 5 sections
...
toggle the comments rail and theme; a spec-type doc also shows a meta strip (slug, version, story/AS
counts, draft, url).
...
- **When:** it renders
- **Then:** the TOC and comments rail are not inline; a comment FAB shows the annotation count and opens the rail as a drawer; tapping a highlight opens the rail
...
- `ViewerScreen` `[N]` *(React route `/d/:slug`; replaces the server-rendered `/d` page — direction B)*
  - `ViewerTopBar`: outline-toggle *(drawer mode)* · back *(non-public)* · brand · title · `LiveBadge` · `FormatBadge` · `VersionButton` *(opens version history — versioning-diff-ui, sibling)* · `CommentsToggle` · `ShareButton` *(opens share — sharing-permissions-ui, sibling)* · `ThemeToggle` · `OverflowMenu`
  - `MetaStrip` *(spec docs only, desktop: slug · version · updated · stories · AS · Draft · url)*
  - `ViewerBody` *(3-pane; collapses to drawers <1200 TOC / <600 rail)*
    - `TocSidebar`: `TocSearch` · `TocGroup` → `TocItem` *(scroll-spy active; `PriorityBadge` if spec)*
    - `DocPane`
      - `DocModeToolbar`: Select·Markup · Wide·Focus *(Markup mode = commenting spec; Select/width here)*
      - `MarkdownView` *(kind=markdown; app-origin render; `AnnotationHighlight` spans paired by anchor)*
      - `HtmlSandboxFrame` *(kind=html; iframe `src="/v/:id"` sandbox allow-scripts; highlights via the bridge — commenting spec)*
      - `ImageViewer` *(kind=image; zoom/pan; `ImageRegionLayer` — suggest-image spec)*
    - `AnnotationsRail`
...
Stories=6, AS=17 — both under the soft target. This is sub-spec 1 of a 3-way by-flow split of
