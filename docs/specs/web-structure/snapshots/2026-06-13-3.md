...
**Date:** 2026-06-13
**Ref:** --
**Reason:** M4 — S-010 flow expanded: cross-cutting feature-root tests move to `<feature>/tests/`; AS-031 + C-011 updated to allow `tests/`.
...
AS-011: S-003 touches only the two largest features
- **Given:** `sharing`, `auth`, `workspaces` are NOT in S-003's scope (they are sub-layered later, in S-006)
- **When:** S-003 completes
- **Then:** S-003 introduces `components/`/`hooks/` only under `viewer` and `docs`; it leaves `sharing`/`auth`/`workspaces` untouched (S-006 sub-layers those)
- **Data:** after S-003, `features/sharing/` is still flat — it changes in S-006, not here
...
### S-006: Sub-layer the remaining features for uniformity (P2)

**Description:** As a developer, every feature folder follows the same internal shape —
UI under `components/`, hooks under `hooks/`, service `client.ts` + `types.ts` + plain
helpers at the root — so the structure is uniform across the whole app, not just the two
largest features. This reverses the earlier "small features stay flat" decision at the
product owner's request (uniformity valued over the minor over-engineering of sub-folders
for small features).
**Source:** user decision 2026-06-13 — "sub-layer tất cả feature" (reverses AS-011's original "leave flat").

**Execution:**
- `depends_on:` S-003, S-005
- `parallel_safe:` false
- `files:` `src/features/{sharing,auth,workspaces}/**` (source + the tests S-005 co-located there), their importers
- `autonomous:` true
- `verify:` `cd apps/web && bun typecheck && bun test` green; each of `sharing`/`auth`/`workspaces` has a `components/` dir; `client.ts`/`types.ts` stay at the feature root

**Acceptance Scenarios:**

AS-020: `sharing` sub-layered
- **Given:** `features/sharing` is flat (UI `.tsx` + `client.ts` + co-located tests, no `use-*` hooks)
- **When:** the sub-layering runs
- **Then:** UI `.tsx` (and their co-located tests) move under `sharing/components/`; `client.ts` and the `*-repo`/state helpers stay at the feature root; no empty `hooks/` is created when there are no hooks; importers + `mock.module` targets updated; suite green
- **Data:** `sharing/share-dialog.tsx` → `sharing/components/share-dialog.tsx`; `sharing/client.ts` unchanged path

AS-021: `auth` sub-layered
- **Given:** `features/auth` is flat (UI `.tsx` + `client.ts` + `zod-resolver.ts` helper + co-located tests)
- **When:** the sub-layering runs
- **Then:** UI `.tsx` (and co-located tests) move under `auth/components/`; `client.ts` + `zod-resolver.ts` stay at root; importers updated; suite green
- **Data:** `auth/sign-in-screen.tsx` → `auth/components/sign-in-screen.tsx`

AS-022: `workspaces` sub-layered
- **Given:** `features/workspaces` is flat (UI `.tsx` + `use-bootstrap.ts` hook + `client.ts`/`types.ts`/`query-keys.ts` + co-located tests)
- **When:** the sub-layering runs
- **Then:** UI `.tsx` (and co-located tests) under `workspaces/components/`, `use-bootstrap.ts` under `workspaces/hooks/`, `client.ts`/`types.ts`/`query-keys.ts` at root; importers + mocks updated; suite green
- **Data:** `workspaces/workspace-switcher.tsx` → `workspaces/components/`; `workspaces/use-bootstrap.ts` → `workspaces/hooks/`

AS-023: No dangling import after the moves (boundary)
- **Given:** the three features' files moved
- **When:** the suite runs
- **Then:** no production or test file references an old flat path for a moved file — `bun typecheck` + `bun test` green, no unresolved import
- **Data:** post-move grep for the old paths returns nothing

### S-007: The API client lives under `services/` (P2)

**Description:** Each feature's `client.ts` (its Eden request thunks — the layer that calls the
API / any 3rd party) lives in a `services/` subfolder, since that is its role.
**Source:** user decision 2026-06-13 — "nếu nó là service gọi api, 3rd party thì nó phải nằm trong service".

**Execution:**
- `depends_on:` S-006
- `parallel_safe:` false
- `files:` `src/features/{auth,sharing,workspaces,docs,viewer}/client.ts` → `…/services/client.ts`; importers; every `mock.module("@/features/<f>/client", …)` in tests
- `autonomous:` true
- `verify:` `cd apps/web && bun typecheck && bun test` green; each feature has `services/client.ts`; no `@/features/<f>/client` import remains

**Acceptance Scenarios:**

AS-024: Every feature's client moves into `services/`
- **Given:** each feature has `client.ts` at its root
- **When:** the move runs
- **Then:** `client.ts` lives at `<feature>/services/client.ts`; every importer references `@/features/<f>/services/client`; typecheck + test green
- **Data:** `features/docs/client.ts` → `features/docs/services/client.ts`

AS-025: Test mocks follow the new service path (boundary)
- **Given:** tests mock the client via `mock.module("@/features/<f>/client", …)`
- **When:** the move runs
- **Then:** every such mock target updates to `@/features/<f>/services/client` and still intercepts; suite green; no old `@/features/<f>/client` path remains anywhere in `apps/web`
- **Data:** `mock.module("@/features/docs/client", …)` → `…/services/client`

### S-008: Feature types live under `types/` (P2)

**Description:** A feature's TypeScript types live in a `types/` subfolder rather than a single
`types.ts` at the root — uniform across features that have shared types.
**Source:** user decision 2026-06-13 — "Tất cả → types/ folder".

**Execution:**
- `depends_on:` S-006
- `parallel_safe:` false
- `files:` `src/features/{workspaces,docs,viewer}/types.ts` → `…/types/` (e.g. `types/index.ts`); importers
- `autonomous:` true
- `verify:` `cd apps/web && bun typecheck && bun test` green; `docs`/`workspaces`/`viewer` have a `types/` dir

**Acceptance Scenarios:**

AS-026: `types.ts` becomes a `types/` folder
- **Given:** `docs`, `workspaces`, `viewer` each have `types.ts` at root
- **When:** the conversion runs
- **Then:** each becomes `<feature>/types/` (reachable at `@/features/<f>/types`); importers resolve; typecheck + test green
- **Data:** `features/docs/types.ts` → `features/docs/types/index.ts`

AS-027: Features with no shared types get no empty `types/` (boundary)
- **Given:** `auth` and `sharing` have no `types.ts`
- **When:** S-008 completes
- **Then:** no empty `types/` folder is created for them
- **Data:** `features/auth/` has no `types/`

### S-009: Zod schemas live under `schema/` (P2)

**Description:** Inline Zod form schemas are extracted into a `schema/` subfolder for the features
that have them, so validation schemas have one home.
**Source:** user decision 2026-06-13 — "schema/ mỗi feature".

**Execution:**
- `depends_on:` S-006
- `parallel_safe:` false
- `files:` `src/features/{auth,sharing,workspaces}/components/*` (forms with inline `z.object`) → extract to `…/schema/`
- `autonomous:` true
- `verify:` `cd apps/web && bun typecheck && bun test` green; `auth`/`sharing`/`workspaces` have a `schema/` dir; forms import their schema from it

**Acceptance Scenarios:**

AS-028: Inline form schemas extracted to `schema/`
- **Given:** `auth` (sign-in/sign-up), `sharing` (invite-row), `workspaces` (members-screen) define `z.object(...)` inline in their components
- **When:** the extraction runs
- **Then:** each schema moves to `<feature>/schema/` and the component imports it from there; behaviour identical; typecheck + test green
- **Data:** sign-up's `z.object({...})` → `features/auth/schema/`

AS-029: Features with no Zod get no `schema/` (boundary)
- **Given:** `docs` and `viewer` use no Zod
- **When:** S-009 completes
- **Then:** no empty `schema/` folder is created for them
- **Data:** `features/docs/` has no `schema/`

### S-010: Feature-local helpers live under `lib/` (P2)

**Description:** A feature's plain helper/utility modules (not UI, hook, service, type, or schema)
live in a `lib/` subfolder, leaving the feature root with only directories.
**Source:** user request 2026-06-13 — "lib (chứa lib của feature)" + "thư mục con feature chỉ có index (nếu có)".

**Execution:**
- `depends_on:` S-006
- `parallel_safe:` false
- `files:` `auth/zod-resolver.ts`, `workspaces/query-keys.ts`, `viewer/{bridge,place-popover,selection-anchor}.ts` (+ `place-popover.test.ts`) → `…/lib/`; importers
- `autonomous:` true
- `verify:` `cd apps/web && bun typecheck && bun test` green; no loose source `.ts` remains at any feature root

**Acceptance Scenarios:**

AS-030: Feature-local helpers move into `lib/`
- **Given:** `auth/zod-resolver.ts`, `workspaces/query-keys.ts`, `viewer/{bridge,place-popover,selection-anchor}.ts` sit at feature roots
- **When:** the move runs
- **Then:** each lands in `<feature>/lib/` (its co-located test follows); importers reference `@/features/<f>/lib/…`; typecheck + test green
- **Data:** `viewer/bridge.ts` → `viewer/lib/bridge.ts`

AS-031: Feature roots hold only subfolders afterward (boundary)
- **Given:** after S-007..S-010, every movable root file has a home
- **When:** S-010 completes
- **Then:** no feature root contains a loose source `.ts`/`.tsx` — only the `components/ hooks/ services/ types/ schema/ lib/` subdirectories (whichever apply); no per-feature `index.ts` barrel is introduced
- **Data:** `ls features/viewer/*.ts*` (depth-1 files) returns nothing
...
- C-002: No file is renamed to PascalCase — kebab-case file naming is preserved throughout. (AS-009, AS-010, AS-014, AS-020, AS-021, AS-022)
...
- C-009: No empty subfolder is created — `services/`/`types/`/`schema/`/`lib/` appear only where a feature has content for them. (AS-027, AS-029)
- C-010: No per-feature `index.ts` barrel; imports stay explicit deep `@/features/<f>/<sub>/<file>`. (AS-031)
- C-011: After S-007..S-010, each feature root holds only subdirectories — no loose source `.ts`/`.tsx` at a feature root. (AS-031)
...
- ~~Sub-layering small features — deferred~~ **REVERSED 2026-06-13:** the product owner chose uniformity; `sharing`/`auth`/`workspaces` are now sub-layered in S-006.
...
- Per-feature `index.ts` barrels — rejected (S-007..S-010): circular-import risk + tree-shaking cost; deep `@/` imports stay explicit.
- Renaming `client.ts` → `<feature>.service.ts` — out of scope; the `services/` folder carries the role, filename stays `client.ts` (less churn).
- Adding Zod schemas where none exist, or a generic validation layer — S-009 only relocates existing inline form schemas; `docs`/`viewer` (no Zod) get no `schema/`.
...
- Phase 2 (S-007..S-010) subdivides each feature internally: `client.ts`→`services/`, `types.ts`→`types/`, inline Zod→`schema/`, helpers→`lib/`, leaving the feature root with only subfolders. `services/` chosen because `client.ts` IS the API/3rd-party service (user's reasoning). NO per-feature barrel (circular-import / tree-shake cost). Kept in THIS spec (not a sibling file) at the user's request — review burden is only the 4 new stories; S-001..S-006 are frozen/done.

## Spec Sizing Notes

Stories=10 (soft target 7, at the G7 overage ceiling ≤10). AS=30 (soft target 20, at the ceiling ≤30).

The overage is by design, not bloat:
- S-001..S-006 are DONE/frozen (web-structure phase 1: alias, lib/api, sub-layering, test co-location). They are not an active review surface.
- S-007..S-010 (phase 2: per-feature `services/`/`types/`/`schema/`/`lib/`) are 4 lean stories, 2 AS each (one move + one boundary), each AS tracing to exactly one atom. Kept in this spec rather than a sibling at the user's explicit request ("dùng spec cũ").

No bloat — each AS traces to one stated move or boundary. If phase-2 grows, split to a sibling spec.
...
| 2026-06-13 | REVERSED AS-011 + added S-006 (sub-layer sharing/auth/workspaces too) per product-owner decision for uniformity; S-004 now depends on S-006; snapshot 2026-06-13.md | user |
| 2026-06-13 | Added S-007..S-010 (per-feature services/types/schema/lib subdivision) + C-009..C-011 + Spec Sizing Notes; snapshot 2026-06-13-2.md | user |
