...
- `depends_on:` S-002
- `parallel_safe:` false
- `files:` `apps/web/src/app.tsx` (move viewer route out of AuthGuard), `apps/web/src/features/viewer/components/viewer-screen.tsx`, `apps/web/src/features/viewer/components/viewer-top-bar.tsx` (logged-out variant), `apps/web/src/features/viewer/services/client.ts`, `apps/web/src/app/session-expiry-listener.tsx` (exempt viewer queries), new `NoAccessView` component
- `autonomous:` true
...
Stories=6 (target 7 — under). AS=29 (target 20 — in G7 overage range ≤30, hard cap 30).
...
No bloat — each AS traces to one stated atom from a Critical/High finding.
