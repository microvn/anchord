...
- **workspaces.settings** (`jsonb`, declared exception): `{ providers, defaultAccess, branding }`.
  **`defaultAccess`** ∈ `restricted | anyone_in_workspace | anyone_with_link` was conceived as a
  per-workspace default doc access. **As of `doc-access-two-axis` (2026-06-25) it is NO LONGER read at
  publish.** The single `docs.general_access` enum it fed was dropped; a new doc now gets a FIXED
  two-axis default (`workspace_role = commenter`, `link_role = null`) at publish, owned by
  `doc-access-two-axis` (C-007 there) regardless of any per-workspace `defaultAccess`. The field stays
  declared in the settings shape as the reserved seam for a future per-workspace admin knob (Not in
  Scope), but it has NO current effect on publish and no code path reads it.
...
- C-007 (SUPERSEDED 2026-06-25 by `doc-access-two-axis`): originally — every workspace carries
  `settings.defaultAccess`, defaulting to `anyone_in_workspace`, which a publish read to set a new
  doc's `general_access` (the shared-group-space default). That mechanism is RETIRED: `docs.general_access`
  is dropped, and a new doc's access is now the FIXED two-axis default `{workspace_role = commenter,
  link_role = null}` applied at publish — owned by `doc-access-two-axis` C-007, at both the web and MCP
  publish surfaces. The shared-group-space outcome is preserved (workspace members get commenter on new
  docs by default; `restricted` is still a per-doc opt-out, now expressed as `workspace_role = null`),
  but it no longer derives from this field. `settings.defaultAccess` is retained only as a reserved seam
  for a future per-workspace admin knob (Not in Scope) and is currently unread.
...
- `settings.defaultAccess` — declared on the workspace but NO LONGER consumed by any publish path
  (superseded 2026-06-25 by `doc-access-two-axis`). The publish-time new-doc default is now the fixed
  two-axis `{workspace_role = commenter, link_role = null}` owned by `doc-access-two-axis` (C-007 there),
  not a value read from this field. No producer/consumer link remains; the field is a reserved,
  currently-unread seam for a future per-workspace admin knob. ✘ link retired.
...
- **Admin UI to change `settings.defaultAccess` per workspace** — deferred (v0.5+). After
  `doc-access-two-axis` (2026-06-25) the field is no longer read at publish; v0 ships a FIXED new-doc
  default (`workspace_role = commenter`, `link_role = null`, owned by `doc-access-two-axis` C-007) for
  every workspace, and `settings.defaultAccess` remains declared only as a reserved seam for this later
  knob (Confluence-style default space permissions). When the knob lands it will set the per-workspace
  publish default on the two axes, not the dropped `general_access` enum. Private docs in v0 are handled
  per-doc via `restricted` (i.e. `workspace_role = null`), not by flipping the workspace default.
...
| 2026-06-25 | Major (M5+M6) — SUPERSEDED by `doc-access-two-axis`: `docs.general_access` dropped, so publish no longer reads `settings.defaultAccess`. New-doc default is now the FIXED two-axis `{workspace_role=commenter, link_role=null}` owned by doc-access-two-axis C-007 (web + MCP). C-007 here marked superseded; C-001 + AS-001/AS-003 drop the inert `defaultAccess` assertion; Data Model + Linked Fields + Not-in-Scope reconciled — field retained as an unread reserved seam for the future per-workspace admin knob. | doc-access-two-axis cascade |
