...
- **notifications table [E→change]** (`apps/backend/src/db/schema.ts:405`): `id`, `userId`, `type`,
  `refId`, `read` (default false), `createdAt`, **+ `comment_id`** (nullable FK → `comments`,
  `onDelete: set null`) — the triggering comment for a comment-type row (S-006/AS-027/028). Set at
  emit for `reply`/`new_feedback`/`thread_activity`; null for non-comment types and when the comment
  is later removed (the read then degrades to the generic per-type summary, C-014). Index
  `notifications_user_read_idx` on `(userId, read)` backs unread queries. (`emailed_at` is still Phase 2.
  The earlier "no new column in v0" is superseded — the panel-enrichment column lands now, 2026-06-21.)
...
- **Read API surface** is **session-scoped to the user, not workspace-scoped** — a notification is
  ...
  **Panel enrichment (2026-06-21):** each row ALSO carries `docTitle` (joined from `refId`→annotation
  →doc, for any doc-scoped row; null for a non-doc row like `invited`), and — for a comment-type row
  (`reply`/`new_feedback`/`thread_activity`) — `actorName` + `snippet` joined from `comment_id`:
  `actorName` = the commenter's display name (the member's `name`, or the guest's `guest_name` for a
  guest commenter), `snippet` = a short excerpt of the comment `body`. All null-safe: a non-comment
  row, or one whose comment/doc is gone, omits them and the client renders the generic per-type
  summary. `snippet` is served IN-APP ONLY — never put in email (C-012 keeps email body-free).
...
**Applies Constraints:** C-008, C-009, C-010, C-014

**Execution:**
- `depends_on:` none
- `parallel_safe:` false
- `files:` `apps/backend/src/routes/notifications.ts` (list paginated, unread-count, mark-read,
  mark-all-read — user-scoped), `apps/backend/src/db/schema.ts` (notifications += `comment_id`) +
  a drizzle migration, `apps/backend/src/notify/{notify.ts,repo.ts,read-repo.ts}` (emit threads the
  triggering `commentId` into `insertNotification`; the read joins `docs.title` + the comment's
  author/body), the comment-create routes `apps/backend/src/routes/annotations.ts` (pass `commentId`
  into the notify dispatchers), `apps/backend/src/index.ts`,
  `apps/web/src/features/notifications/components/notification-panel.tsx` + `lib/format.ts`
  (`summaryFor` becomes a per-type template with title/actor/snippet slots), `apps/web/src/features/notifications/services/client.ts` (row type += docTitle/actorName/snippet)
- `autonomous:` true
- `verify:` signed-in user with mixed read/unread rows → the list returns their rows newest-first, one bounded page; the unread count matches; clicking an unread item flips only it; mark-all clears the badge; another user's rows are never returned. A reply row reads "<name> replied in <doc title>" + a comment excerpt; an `invited` row shows no excerpt.
...
AS-026: A doc-scoped row carries its doc title
- **Given:** Bob has a notification whose `refId` resolves to an annotation on the doc titled "Refund Spec"
- **When:** Bob lists notifications
- **Then:** that row carries the doc title "Refund Spec" (joined from the row's annotation→doc in the same read) so the panel reads "… in Refund Spec" without a second fetch; a row that resolves to no doc carries no title
- **Data:** refId → annotation on "Refund Spec"; one `invited` row with no doc → no title
- **Setup:** a doc-scoped row + a non-doc row

AS-027: A comment-type row carries the actor's display name from the triggering comment
- **Given:** a `thread_activity` row whose triggering comment was written by the member "Mara", and another whose commenter is a guest with session name "swift-otter-k7m2"
- **When:** Bob lists notifications
- **Then:** the first row carries actor "Mara" and the second carries "swift-otter-k7m2" — the commenter's display name from the triggering comment (the member's name, or the guest's session name for a guest commenter)
- **Data:** member commenter "Mara"; guest commenter "swift-otter-k7m2"

AS-028: A comment-type row carries a short excerpt of the triggering comment
- **Given:** a `new_feedback` row whose triggering comment body is "can we cap the partial refund at 50% of the original charge"
- **When:** Bob lists notifications
- **Then:** the row carries a short excerpt of that body (truncated for display); the excerpt is served IN-APP ONLY and is never placed in any email (C-012 keeps email body-free)
- **Data:** comment body ~90 chars → a truncated excerpt

AS-029: A non-comment-type row carries no actor/excerpt and renders the generic summary
- **Given:** an `invited` row (no triggering comment) and a `thread_activity` row whose triggering comment was since removed (comment_id now resolves to nothing)
- **When:** Bob lists notifications
- **Then:** neither row carries an actor or excerpt; each renders its generic per-type summary (the enrichment degrades safely, never erroring on an absent comment/doc)
- **Data:** an `invited` row; a comment-type row whose comment was deleted
...
- C-014: **In-app row enrichment is null-safe + in-app only** — the panel row's `docTitle`
  (refId→annotation→doc) and, for a comment-type row, `actorName` + `snippet` (from `comment_id`)
  are computed in the SAME list read; any that cannot resolve (non-doc row, non-comment row, or a
  deleted comment/doc) is omitted and the row renders its generic per-type summary — never an error.
  The `snippet` is served to the in-app panel ONLY and is never placed in email (defers to C-012,
  email body-free). (AS-026, AS-027, AS-028, AS-029)
...
Stories=7 (target 7, at soft target). AS=28 (target 20, 8 over — in G7 overage range ≤30).

The over-target AS come from G1 splits (one stated atom each, not bloat):
- S-006 panel enrichment: 4 AS for 4 distinct served fields/rules — doc title (AS-026), actor from
  the triggering comment (AS-027), comment excerpt in-app only (AS-028), and the null-safe non-comment
  /deleted-comment degrade (AS-029). Each is a distinct field or a distinct degrade rule; merging
  would mix the served-field assertions with the fallback.
...
| 2026-06-21 | Major (M1+M6, snapshot 2026-06-21-panel-enrichment): in-app notification panel enrichment — S-006 += AS-026 (doc title), AS-027 (actor from the triggering comment — member name or guest session name), AS-028 (comment excerpt, in-app only per C-012), AS-029 (non-comment / deleted-comment → null-safe generic). Data Model: notifications += `comment_id` (nullable FK→comments, set-null) — set at emit for comment-types, threaded from the comment-create route through notify → insertNotification; read joins docs.title + the comment author/body. + C-014 (enrichment null-safe + in-app only). Supersedes the "no new column in v0" Data Model note. Source: panel-is-information-poor review (generic per-type strings, no doc/actor/snippet). | -- |
