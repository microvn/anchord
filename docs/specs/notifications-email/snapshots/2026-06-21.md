...
**Date:** 2026-06-21
**Ref:** --
**Reason:** M1 — new AS-025 (slug-unresolvable email guard) added to S-007
...
- C-013: **Deep-link anchor** — the email link is `{APP_URL}/d/{slug}#annotation-{id}`; the viewer
  ...
  the existing in-app focus path (`focusThread`/`scrollToAnno`, `data-anno` marks). (AS-019, AS-024)
...
- GAP-002 (status: resolved): owner-also-on-detach — decided 2026-06-20 to **keep recipients =
  ...
  detach …".
- GAP-003 (status: open): unread-**badge cap** in the UI ("9+"?) and the unread-count **poll
  interval**. Build-time UI detail; not behaviour-shaping. Source: "Unread-badge cap in the UI
  ('9+'?) and poll interval".
- GAP-004 (status: open): plain-text email **body wording** per event type (one-liner + deep-link
  line). AS-019 asserts the minimal-safe outcome (plain text + absolute deep-link); the exact
  per-event copy is deferred to build time. Source: "Plain-text email body shape — one-liner per
  event type …".
...
Stories=7 (target 7, at soft target). AS=23 (target 20, 3 over — in G7 overage range ≤30).
...
- S-006 read surface: 7 AS for 7 distinct endpoints/rules — list (AS-012), unread-count (AS-013),
  ...
  intents.
- S-007 email delivery: 5 AS for 5 atoms — email-sent-with-deep-link (AS-019), transport-fail
  backoff/dead-letter (AS-020), low-signal-no-email channel policy (AS-021), boot config validation
  (AS-022), viewer honours the deep-link fragment (AS-024). The email and the viewer halves are one
  vertical slice (build the link + scroll to it), kept whole to avoid orphaning the deep-link seam.

(AS-011 was the dropped `invite_accepted` scenario — GAP-006; its ID is retired, not reused.
AS-024 is the latest sequential ID.)
