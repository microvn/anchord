...
**Date:** 2026-06-24
**Ref:** --
**Reason:** M5/M6 — doc-share `invited` reverted to in-app-only (RESTORES this spec's original assertion; reverses the 2026-06-23 in-app + email upgrade). The transactional invite email is the only email; the notification channel never emails for `invited`.
...
- **`notification_type` pgEnum [E]** (`schema.ts:403`, currently `["reply"]`): extended additively
  ...
  **Email eligibility is derived from `type`** as a BASE default — not a stored column. (As of
  2026-06-23 the doc-share `invited` type defaults email-eligible; `resolved`/`detached` stay in-app
  only. The authoritative per-user-overridable channel policy is now owned by `notification-preferences`
  — its supported-channel matrix + per-user prefs + master email switch supersede the flat
  type-derived rule. See C-006.)
...
AS-010: Being invited creates an in-app row plus a notification email for the invitee
- **Given:** Alice shares a doc with dev@acme.com, who has an account, and dev@acme.com has default (unchanged) preferences
- **When:** the invite is created
- **Then:** the invitee gets one in-app row AND one notification email — the doc-share `invited` event defaults to in-app + email (retired the in-app-only behavior 2026-06-23, per `notification-preferences:C-003`/`AS-006`); a user who turns the invited email off (or the master email switch off) gets in-app only
- **Data:** invitee dev@acme.com, default preferences
...
- C-006: **Channel policy by signal** — high-signal events (new_feedback, thread_activity,
  suggestion_decided) send email + in-app; low-signal events (resolved, detached) send in-app only.
  The doc-share `invited` event defaults to **in-app + email** (upgraded from in-app-only —
  retired 2026-06-23). Email eligibility is derived from the notification `type` as a BASE default;
  the authoritative, per-user-overridable channel policy is now owned by `notification-preferences`
  (its supported-channel matrix + per-user prefs + master email switch, `notification-preferences:C-003`).
  This spec defines the base event taxonomy + default channels; `notification-preferences` refines and
  gates them. (AS-001, AS-008, AS-010, AS-019, AS-021)
...
- **Doc-share `invited` upgraded to in-app + email** (cross-spec resolution of an S2 raised during
  `notification-preferences` S-002). `notification-preferences:C-003`/`AS-006` makes the doc-share
  `invited` event default to in-app + email (its supported-channel matrix marks `invited.email = on`),
  and that spec now owns the authoritative, per-user-overridable channel policy. This spec's earlier
  "`invited` is low-signal / in-app only" assertion (C-006, AS-010, Data Model enum note) is retired
  to match. `resolved`/`detached` remain in-app-only. NOTE: the notification email for doc-share
  invited is delivered through the notify path — the route wiring (`routes/sharing.ts notifyOnInvited`
  currently passes a no-op mail enqueuer + no `APP_URL`) must be completed for this to send
  end-to-end; tracked as a build follow-up.
...
| 2026-06-23 | Major (M6, snapshot 2026-06-23): retired the "doc-share `invited` is low-signal / in-app only" assertion — C-006 + AS-010 + Data Model enum note now have `invited` defaulting to in-app + email, with the authoritative channel policy owned by `notification-preferences` (matrix + per-user prefs + master switch, `notification-preferences:C-003`/`AS-006`). `resolved`/`detached` stay in-app only. Resolves the S2 raised in notification-preferences S-002. + Clarifications 2026-06-23. | notification-preferences:C-003/AS-006 |
