...
**Date:** 2026-06-19
**Ref:** --
**Reason:** M6 (C-005 Origin policy reworded: present-but-not-allowlisted → reject, absent → allow for CLI clients; transport now SDK web-standard Streamable HTTP) + M5 (AS-001 Then expanded for the MCP handshake) + 2 new AS (AS-030/031 origin behavior)
