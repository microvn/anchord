...
**Date:** 2026-06-19
**Ref:** --
**Reason:** M6 — C-011 default-project enforcement clause refined (partial-unique index confirmed as the mechanism + owner_id IS NOT NULL precision; resolves the build-time S2 conflict with workspace-project's "no partial index" comment)
...
- C-005: Transport is Streamable HTTP at `/mcp` (a single endpoint; the token carries its
  workspace, so no workspace is in the path). The Origin header is validated against an allowlist
  = the configured base-URL origin; a request with an absent/`null` Origin is rejected
  (DNS-rebinding). The endpoint is exempt from the API envelope — but exemption is by mount
  point: the envelope is opt-in via `apiEnvelope()`, so `/mcp` MUST be mounted OUTSIDE any
  enveloped group (and outside the shared parent whose scoped hooks propagate); responses are
  raw JSON-RPC. (AS-001, AS-023)
...
- C-011: Write-path concurrency/atomicity — version creation is serialized per-doc (advisory
  ...
  default-project resolution is idempotent under concurrent create, DB-enforced by
  `UNIQUE(workspace_id, owner_id) WHERE is_default` (exactly one default per owner per
  workspace). (AS-026, AS-027)
...
Stories=6 (target 7 — under). AS=28 (target 20 — in the G7 overage range ≤30, G1-driven;
highest id AS-029 since removed AS-012 is not reused).
...
- S-001: +AS-022 (revoke mid-session), AS-023 (raw JSON-RPC), AS-024 (rate-limit throttle),
  AS-025 (per-user token cap) — distinct auth/transport atoms.
