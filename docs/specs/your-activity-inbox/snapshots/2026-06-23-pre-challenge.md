...
**Reason:** M1 (+AS-019), M4/M5 (Then changes), M6 (+C-007/C-008/C-009, +Linked Fields), Data Model change — security/assumption hardening; inline actions kept in v0.
...
**Shared page shell:** this spec builds the `/me/activity` page shell + the two-tab container (**For you** | **Your actions**) and fills the **For you** tab. The **Your actions** tab is a placeholder here and is filled by the sibling spec `your-activity-actions` (2b), which depends on the `workspace-activity` `activity` table. Coordinate the shell once (here); 2b only mounts its tab.
...
No new tables. One additive enrichment to the existing notification read surface (`GET /api/me/notifications`):

- **`workspaceId` + `workspaceName`** added per returned item — the workspace that owns the notification's target, so the inbox renders a per-item workspace chip across workspaces. Derived: for doc/annotation types via `annotation → doc → project → workspace`; for `workspace_*` types `refId` already IS the workspace id (and `refLabel` already snapshots the name). Read-time only; no schema change.

Everything else (the `notifications` table, types, read/unread state) is unchanged and owned by `notifications-email`.
...
- **When:** Mara opens it
- **Then:** that item becomes read and the unread count decreases by one
...
AS-012: "Open in doc" jumps to the source
- **Given:** a reply notification whose `refId` is an annotation thread
- **When:** Mara clicks "Open in doc"
- **Then:** the doc opens at that annotation (deep-link), falling back to the doc top if the anchor no longer resolves
- **Data:** annotation on "§ Path scoping"
...
- `depends_on:` S-003
- `parallel_safe:` false
- `files:` apps/web/src/features/your-activity/components/ (invite actions; call `POST /api/invitations/:id/accept` and `/reject`)
- `autonomous:` true
- `verify:` accept an invite item → the workspace appears in the switcher at the invited role; decline → the invite is gone and does not activate later.
...
C-006: Day grouping is rendered client-side over the flat paged list (a day may straddle a page boundary; same-day headers merge across pages); "Today/Yesterday" are computed in the VIEWER's timezone. The list is paged via the existing notification read (no realtime); occasional page-drift on consecutive fetches is acceptable (manual surface). (AS-002)
...
- **Invite accept/decline** `apps/backend/src/routes/workspaces.ts` (`POST /api/invitations/:id/accept`, `/reject`) — both exist; token bound to the caller's email.
- **Mark/unread endpoints** `apps/backend/src/routes/notifications.ts` (`/:id/read`, `/read-all`, `/unread-count`) — reused as-is.
- **Risk — shared page shell with 2b:** `/me/activity` + the tab container is built here; 2b only mounts its "Your actions" tab. If 2b ships first the shell moves there — coordinate so the shell is built exactly once.
